...
<2> v3 only: `middle/section@removeInRFC`, `middle/section@toc`, `middle/section@numbered` (attributes only available in v3)
<3> v3: `middle/section/name`; v2: `middle/section@title`
<4> `middle/section/t`
<5> v3: `middle/section/section/name`; v2: `middle/section/section@title`
<6> `middle/section/section/t`
<7> v3: `middle/section/section/section/name`; v2: `middle/section/section/section@title`
...
<7> v3 only: `<bcp14>MUST NOT</bcp14>`. Not supported in v2.
...
<9> `figure/artwork@src`, `figure/artwork@align`, `figure/artwork@alt`; `figure/artwork@type` (only available in v2, intended to be a MIME type; v3: populated as either `svg` or `binary-art` depending on file suffix)
...
| http://asciidoctor.org/docs/user-manual/#verse[Verse] | Rendered as normal paragraph | Supported
...
| Formatting: http://asciidoctor.org/docs/user-manual/#discrete-headings[Floating Title] 2+| Treated as inline section
