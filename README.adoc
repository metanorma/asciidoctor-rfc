...
Its syntax is designed to be "`native-asciidoctor`" as much as possible, with some
templated use of attributes to convey added information for RFC XML output.
We refer to the version of Asciidoc markup processed by this gem as AsciiRFC.
See also https://github.com/riboseinc/rfc-asciidoc-rfc for our Internet Draft
documenting this use of Asciidoc; this Readme documents the markup used in full
detail.
...
|`:flush-biblio:`
|Optional. Default value `false`. Allowed values: `true`, `false`.
Delete and reload the cache of references to be included externally, during processing of this document.
The cache is stored in `~/.asciidoc-rfc-biblio-cache.json`.
...
The additional document option `rfc2629xslt` (default value: false) injects into the document header the processing
...
----
--
<1> v3: `figure/sourcecode@anchor`; v2: Not supported: use explicit figure wrapper
...
....
--
<1> v3 only: `figure/artwork@anchor`; v2: Not supported: use explicit figure wrapper
...
<3> `figure/name`
<4> v2 only: `figure/preamble` (only available in v2)
<5> v3 only: `figure/artwork@anchor`
...
The cache is not rebuilt daily, as it is with xml2rfc. If you want to refresh the cache, either delete
your `~/.asciidoc-rfc-biblio-cache.json` file; insert the document attribute `:flush-biblio: true` into
the header of the document being processed; or run the asciidoctor executable with option `-a flush-biblio=true`
