...
NOTE: Asciidoctor will attempt to match a crossreference to a section title, if it does not find
a matching anchor ID. (This behaviour will become optional in Asciidoctor 1.5.7.) 
...
Asciidoctor will currently pick the latter as the target of the reference, even if that section
...
To prevent this, make sure that references have distinctive anchors that cannot be
conflated with section titles.
