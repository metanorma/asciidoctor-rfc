...
[[id]] <5>
[spacing=compact,start=n,group=n,counter=token,hang-indent=n,arabic|loweralpha|upperralpha|lowerroman|upperroman] <6>
. A <7>
. B <7>
--
...
<6> v2: `list/counter`, `list@style`, `list@hangIndent` (`start`, `empty` and `group` not available) v3: `ol@empty`, `ol@start`, `ol@group`, `ol@type` (`counter`, `hangIndent` not available)
